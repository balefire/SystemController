...
public class HexConverUtils {
    ...
    public static byte[] CRC_16(byte[] str, int DataLen)
    {
        byte[] r = new byte[2];

        byte uchCRCHi = (byte) 0xFF ; /* high byte of CRC initialized */
        byte uchCRCLo = (byte) 0xFF ; /* low byte of CRC initialized */
        int uIndex = 0 ; /* will index into CRC lookup table */
        byte uIndex2 = 0 ;
        while (DataLen-- > 0)/* pass through message buffer */
        {
            uIndex = (uchCRCHi ^ str[uIndex2 ++]) & 0xFF; /* calculate the CRC */
